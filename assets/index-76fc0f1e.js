...
fn sample_data_at_location(xf: f32, yf: f32) -> f32 {
    ...
    return total / count;
}`;class V{constructor(e){c(this,"device");this.device=e}getFloat32Buffer(e,n,s=0){let t=s;e==="uniform"?t|=GPUBufferUsage.UNIFORM:t|=GPUBufferUsage.STORAGE,(Array.isArray(n)||n instanceof Float32Array)&&(t|=GPUBufferUsage.COPY_DST);const r=typeof n=="number"?n:n.length,o=this.device.createBuffer({size:r*4,usage:t});return n instanceof Float32Array?this.device.queue.writeBuffer(o,0,n):Array.isArray(n)&&this.device.queue.writeBuffer(o,0,new Float32Array(n)),{type:"buffer",buffer:o,binding:e}}getTextureView(e,n){return{type:"texture",view:this.device.createTexture({size:{width:e,height:n},format:"rgba8unorm",usage:GPUTextureUsage.COPY_DST|GPUTextureUsage.STORAGE_BINDING|GPUTextureUsage.TEXTURE_BINDING}).createView()}}getCanvasContext(e){const n=e.getContext("webgpu");if(!n)throw Error("No GPU context available");return n.configure({device:this.device,format:"bgra8unorm",alphaMode:"opaque"}),n}getCommandRunner(e){return()=>{const n=this.device.createCommandEncoder();for(const s of e)s(n);this.device.queue.submit([n.finish()])}}}const R=(a,e=window.innerWidth,n=window.innerHeight)=>{const s=window.devicePixelRatio||1;a.width=e*s,a.height=n*s,a.style.width=e+"px",a.style.height=n+"px"};class v{constructor(e,n,s,t){c(this,"pipeline");c(this,"bind_group");c(this,"runner",(e,n=1,s=1)=>t=>{const r=t.beginComputePass();r.setPipeline(this.pipeline),r.setBindGroup(0,this.bind_group),r.dispatchWorkgroups(e,n,s),r.end()});const r=e.createBindGroupLayout({entries:t.map((i,l)=>({binding:l,visibility:GPUShaderStage.COMPUTE,buffer:i.type==="buffer"?{type:i.binding}:void 0,storageTexture:i.type==="texture"?{access:"write-only",format:"rgba8unorm",viewDimension:"2d"}:void 0}))});this.bind_group=e.createBindGroup({layout:r,entries:t.map((i,l)=>({binding:l,resource:i.type==="texture"?i.view:{buffer:i.buffer}}))});const o=e.createPipelineLayout({bindGroupLayouts:[r]});this.pipeline=e.createComputePipeline({layout:o,compute:{module:e.createShaderModule({code:n}),entryPoint:s}})}}var b=`@group(0) @binding(0) var screen_sampler: sampler;
...
}`;class k{constructor(e,n,s){c(this,"pipeline");c(this,"bind_group");c(this,"getTarget");c(this,"renderer",e=>{const n=e.beginRenderPass({colorAttachments:[{view:this.getTarget(),loadOp:"clear",storeOp:"store"}]});n.setPipeline(this.pipeline),n.setBindGroup(0,this.bind_group),n.draw(6,1,0,0),n.end()});this.getTarget=s;const t={addressModeU:"repeat",addressModeV:"repeat",magFilter:"linear",minFilter:"nearest",mipmapFilter:"nearest",maxAnisotropy:1},r=e.createSampler(t),o=e.createBindGroupLayout({entries:[{binding:0,visibility:GPUShaderStage.FRAGMENT,sampler:{}},{binding:1,visibility:GPUShaderStage.FRAGMENT,texture:{}}]});this.bind_group=e.createBindGroup({layout:o,entries:[{binding:0,resource:r},{binding:1,resource:n}]});const i=e.createPipelineLayout({bindGroupLayouts:[o]});this.pipeline=e.createRenderPipeline({layout:i,vertex:{module:e.createShaderModule({code:b}),entryPoint:"vert_main"},fragment:{module:e.createShaderModule({code:b}),entryPoint:"frag_main",targets:[{format:"bgra8unorm"}]},primitive:{topology:"triangle-list"}})}}class p{constructor(e,n,s){c(this,"runner");c(this,"writeSceneValue");c(this,"render",e=>{this.writeSceneValue(0,e),this.runner()});c(this,"setAcceleration",e=>this.writeSceneValue(16,e));c(this,"setVelocity",e=>this.writeSceneValue(20,e));c(this,"setSensor",e=>this.writeSceneValue(24,e));c(this,"setRange",e=>this.writeSceneValue(28,e));c(this,"setHalflife",e=>this.writeSceneValue(32,e));const{thousands:t,acceleration:r,velocity:o,sensor:i,range:l,halflife:I}=s,G=t*1e3,u=new V(n),P=u.getCanvasContext(e),S=[0,0,e.width,e.height,r,o,i,l,I],C=[...Array(G)].flatMap(()=>{const h=Math.sqrt(Math.random())*Math.min(e.height,e.width)*.4,d=Math.random()*Math.PI*2;return[e.width/2+h*Math.sin(d),e.height/2+h*Math.cos(d),(Math.PI+d)%(Math.PI*2),0]}),y=u.getTextureView(e.width,e.height),x=u.getFloat32Buffer("uniform",S),T=u.getFloat32Buffer("storage",C),A=u.getFloat32Buffer("storage",e.width*e.height*4),g=[y,x,T,A];this.writeSceneValue=(h,...d)=>n.queue.writeBuffer(x.buffer,h,new Float32Array(d));const D=new v(n,m,"fade_values",g),B=new v(n,m,"update_and_draw_points",g),E=new v(n,m,"draw_to_texture",g),M=new k(n,y.view,()=>P.getCurrentTexture().createView());this.runner=u.getCommandRunner([D.runner(e.width,e.height),B.runner(t,1e3),E.runner(e.width,e.height),M.renderer])}static async from(e,n){var r;const s=await((r=navigator.gpu)==null?void 0:r.requestAdapter()),t=await(s==null?void 0:s.requestDevice());if(!t)throw new Error("No GPU device found!");return new p(e,t,n)}}class _{constructor(e,n,s){c(this,"canvas");c(this,"controls");c(this,"config");c(this,"runner");this.runner=e,this.canvas=n,this.config=s;const t=document.createElement("div");t.setAttribute("style","display: flex");const r=document.createElement("input");r.setAttribute("style","flex-shrink: 1; min-width: 0;"),r.defaultValue=""+s.thousands;const o=document.createElement("button");o.innerText="Restart",o.onclick=()=>{const i=Number(r.value||s.thousands);i&&!isNaN(i)&&(this.config.thousands=i,this.restart())},t.append(r,o),this.controls=[t,f("Acceleration",s.acceleration,i=>{this.config.acceleration=i,this.runner.setAcceleration(i)}),f("Velocity",s.velocity,i=>{this.config.velocity=i,this.runner.setVelocity(i)}),f("Sensor",s.sensor,i=>{this.config.sensor=i,this.runner.setSensor(i)}),f("Range",s.range,i=>{this.config.range=i,this.runner.setRange(i)}),f("Halflife",s.halflife,i=>{this.config.halflife=i,this.runner.setHalflife(i)})]}static async from(e,n=L){R(e);const s=await p.from(e,n);return new _(s,e,n)}update(e){this.runner.render(e)}restart(){p.from(this.canvas,this.config).then(e=>this.runner=e)}}const L={thousands:400,acceleration:3,velocity:200,sensor:10,range:2,halflife:.1},f=(a,e,n)=>{const s=document.createElement("div");s.setAttribute("style","display: flex");const t=document.createElement("p");t.setAttribute("style","flex-grow: 1; margin: 0"),t.innerText=a+":";const r=document.createElement("input");r.setAttribute("style","width: 30px; text-align: right;"),r.defaultValue=""+e;const o=()=>{const i=Number(r.value);isNaN(i)||n(i)};return r.onblur=o,r.onkeydown=i=>{i.code==="Enter"&&o()},s.append(t,r),s},w=document.getElementById("canvas");_.from(w).then(a=>{N(a),window.simulation=a}).catch(()=>{var e;const a=document.createElement("h4");a.innerText="This app does not work without Web GPU - try running Chrome Canary or Firefox Nightly, and enabling Web GPU",(e=w.parentElement)==null||e.appendChild(a)});
